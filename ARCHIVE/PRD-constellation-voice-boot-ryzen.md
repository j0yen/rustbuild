...
**Depends on:** PRD-constellation-wm-daemons-ryzen (wm-* stack running on ryzen7)

## TL;DR

With wintermute daemons running on ryzen7, the final step is voice-on-boot:
...
- ryzen7 has i3 in the repo (`~/wintermute/constellation/`), but no display manager or autologin configured
...
- ryzen7 has a Radeon Barcelo iGPU (gfx90c) — X11/i3 works fine without discrete GPU
...
```
ssh ryzen7 "sudo pacman -Syu --noconfirm --needed greetd greetd-agreety i3-wm i3status alacritty xorg-server xorg-xinit pipewire pipewire-pulse wireplumber feh"
```

### Step 2 — greetd autologin config

`/etc/greetd/config.toml` on ryzen7:
```toml
[terminal]
vt = 1
...
Enable and start:
```
ssh ryzen7 "sudo systemctl enable --now greetd"
```

### Step 3 — i3 config

Copy `~/.config/i3/config` from wintermute to ryzen7 (the green-phosphor theme,
...
Copy wallpaper:
```
rsync -avz ~/.config/wallpaper/steel-blue.ppm ryzen7:~/.config/wallpaper/
```

### Step 4 — Alacritty config

Copy `~/.config/alacritty/alacritty.toml` from wintermute to ryzen7 (Gruvbox Dark,
...
Create `~/.config/systemd/user/i3.service.d/notify.conf` on ryzen7:
```ini
[Service]
...
wintermute). Verify on ryzen7 after copy. `systemctl --user enable wintermute.target`.

### Step 7 — Smoke test (remote)

After rebooting ryzen7:
```
ssh ryzen7 "systemctl --user is-active wm-audio wm-stt wm-brain"
```
All should be `active` within 30s of boot.

## Acceptance criteria

1. `ssh ryzen7 "systemctl is-active greetd"` returns `active`.
2. After ryzen7 reboot, `ssh ryzen7 "systemctl --user is-active wm-audio"` returns `active` (voice stack auto-started by wintermute.target).
3. `ssh ryzen7 "cat ~/.config/alacritty/alacritty.toml | grep gruvbox" -i` — Gruvbox colors present (identical appearance).
4. `ssh ryzen7 "ls ~/.config/wallpaper/steel-blue.ppm"` — wallpaper copied.
5. `ssh ryzen7 "systemctl --user is-active wm-brain"` returns `active` (brain connected to cloud API tier).
