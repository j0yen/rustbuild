...
The Tailscale mesh is live (wintermute + ryzen-work + hub all connected, confirmed
...
ryzen-work (which already has `~/.local/bin/nats-server` installed) and creates the
...
- ryzen-work: `systemctl --user is-active nats` → inactive; `systemctl --user is-active nats-leaf` → inactive
- ryzen-work: `which nats-server` → `/home/jsy/.local/bin/nats-server` (already installed)
- ryzen-work's `~/.config/nats/leaf.conf` points to `nats://fleet:…@hub:7422` — which resolves to 100.66.158.49 via Tailscale MagicDNS, currently unreachable
...
ryzen-work running NATS in hub mode is the fastest unblock. The hub VPS can take over as
the true hub once it's provisioned (ryzen-work's config can be changed to leaf then).

## What this builds

Runs on: **ryzen-work** (via SSH from wintermute).

1. **`~/.config/nats/hub.conf`** (on ryzen-work) — NATS server config:
   ```
   ...
   ```
   Note: Tailscale IP for ryzen-work is 100.111.184.102 (confirmed live).

2. **`~/.config/systemd/user/nats-hub.service`** (on ryzen-work):
   ```ini
   [Unit]
   Description=NATS hub — constellation fleet (ryzen-work)
...
3. **Update `~/.config/nats/leaf.conf`** (on ryzen-work) — change hub URL from
   `nats://fleet:…@hub:7422` to `nats://127.0.0.1:7422` (same server, leaf
   connection from localhost). This lets the ryzen-work agorabus bridge connect
   via the local leaf port rather than requiring the cloud hub.

4. **Enable and start** `nats-hub.service` on ryzen-work via `systemctl --user enable --now`.
...
1. `systemctl --user is-active nats-hub` returns `active` on ryzen-work.
2. `nats --server nats://127.0.0.1:4222 account info` exits 0 on ryzen-work.
...
4. JetStream is enabled with domain `hub`: `nats --server 127.0.0.1:4222 str ls` exits 0 on ryzen-work.
