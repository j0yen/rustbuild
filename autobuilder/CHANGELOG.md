...
`hermetic-build`'s R2 route-not-local check (`read_route_observed`) scanned `target/autobuilder/route.log` for any line at or after this build's start timestamp carrying the literal field `burst`, with no check on which invocation wrote it. That log is shared with every other gate/branch-agent building the same repo concurrently (same `target/` dir), so a legitimately burst-routed concurrent build blocked THIS build's hermetic-build with cause=route-not-local even though its own cargo child's route was local-pinned (receipt: extended-gates/target/autobuilder/receipts/hermetic-build-receipt.json captured_at 2026-09-18T19:13:42Z on burst-lane). Fix: `read_route_observed` now also requires the line's pid field to equal this build's own cargo child pid (`child.id()` from `spawn_and_sample`) before counting a `burst` line against it — both `cargo_route_log` (build-skill's scripts/lib/cargo-route.sh) and burst-lane-bin/cargo's own `route_log` write `$$` before any `exec` that would change the process image, and the one path that later forks a grandchild (cargo-budget.sh's slot-acquiring `cmd_run`) only runs after that line is already written, so the logged pid is always the top-level shim invocation's own — never a grandchild's — making pid an exact discriminator with no run-id plumbing needed. Five new unit tests in `hermetic_build.rs` cover unknown/local/burst/foreign-pid-burst/stale-own-pid-burst cases.
...
Version bumped patch (0.6.1 -> 0.6.2), not the PRD's default minor: this crate was frozen/ported-from earlier the same day (see `autobuilder/PORTED.md`), with the canonical `~/wintermute/autobuilder` crate deliberately kept at a higher version (v0.7.0) so "the canonical install" stays checkable by version number alone. A minor bump here (0.6.1 -> 0.7.0) would have collided with that invariant; a patch bump keeps this crate strictly below canonical while still recording the fix.
